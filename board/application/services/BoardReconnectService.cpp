#include "BoardReconnectService.hpp"
#include <algorithm>
#include <chrono>
#include <iostream>
#include <thread>
...
namespace board::application::services {
    BoardReconnectService::~BoardReconnectService()
    {
        stop();
        waitForCompletion();
    }

    void BoardReconnectService::startAsync(ConnectFunction connect, OnSuccess onSuccess, OnGiveUp onGiveUp)
    {
        bool expected = false;
    ...
    void BoardReconnectService::stop()
    {
        running_ = false;
    }

    void BoardReconnectService::waitForCompletion()
    {
        if(reconnectThread_.joinable())
        {
            reconnectThread_.join();
